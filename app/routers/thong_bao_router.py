"""
thong_bao_router.py — ThongBao.jsx
-----------------------------------------------------------------
    GET   /api/thong-bao                        danh sách của tôi
    PATCH /api/thong-bao/{id}/da-doc             đánh dấu 1 cái đã đọc
    PATCH /api/thong-bao/danh-dau-tat-ca-da-doc  đánh dấu tất cả đã đọc
          (đây chính là endpoint mà ghi chú đầu file ThongBao.jsx nói
          "nên bổ sung riêng" thay vì gọi PATCH lặp lại nhiều lần)
"""
...
def danh_dau_tat_ca_da_doc(
    ...
    (
        ...
        .filter(
            ...
            models.ThongBao.da_doc == False,  # noqa: E712
        )
        .update({"da_doc": True})
    )
    db.commit()
